...
// تأكد أن الرابط هو رابط السيرفر الخاص بك
...
useEffect(() => {
  const fetchRooms = async () => {
    try {
      ...
      // 1. تنظيف الرابط الأساسي (إزالة /api أو / من نهاية الرابط إن وُجدت لمنع التكرار)
      let baseUrl = import.meta.env.VITE_API_URL || "http://localhost:5000";
      baseUrl = baseUrl.replace(/\/api\/?$/, "").replace(/\/$/, "");

      const res = await fetch(`${baseUrl}/api/chats/rooms`, {
        headers: {
          Authorization: `Bearer ${token}`, // إرسال مفتاح الدخول
        },
      });

      // 2. فحص نوع البيانات الراجعة لمنع خطأ (Unexpected token '<' is not valid JSON)
      ...
      // 3. التأكد من أن الطلب نجح وأن البيانات الراجعة عبارة عن مصفوفة (Array)
      if (res.ok && Array.isArray(data)) {
        setRooms(data);
      } else {
        console.error("Error from server:", data);
        setRooms([]); // إرجاع مصفوفة فارغة في حالة الخطأ لمنع انهيار الصفحة
      }
    } catch (error) {
      console.error("Error fetching rooms:", error);
      setRooms([]); // إرجاع مصفوفة فارغة عند حدوث أي خطأ في الاتصال
    }
  };
  ...
  useEffect(() => {
    // الانضمام لغرفة المشرفين أو الاستماع العام لرسائل كل الغرف لو السيرفر بيدعم ده
    // (لو السيرفر بيبعت رسائل receive_message لكل الناس، الكود هيشتغل. 
    // لو بيبعت للغرفة بس، لازم السيرفر يبعت event تاني للـ Admin يقوله في رسالة جديدة)
    ...
    const handleReceiveMessage = (data) => {
      ...
      // 2. إذا كانت الرسالة لغرفة أخرى، والرسالة من العميل
        ...
        // (اختياري) إضافة الغرفة للقائمة لو كانت جديدة ومش موجودة في rooms
        setRooms(prevRooms => {
    ...
    <div className="flex h-screen bg-gray-100 p-4 gap-4" dir="rtl">
      ...
      <div className="w-1/3 bg-white rounded-2xl shadow-md border border-gray-200 flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto">
          {rooms.length === 0 ? (
            <div className="p-10 text-center text-gray-400 italic">لا توجد محادثات سابقة</div>
          ) : (
            rooms.map(id => {
              ...
              return (
                <button 
                  ...
                  className={`w-full p-5 text-right border-b border-gray-100 transition-all flex justify-between items-center ${
                    activeRoom === id ? 'bg-blue-50 border-r-4 border-blue-600' : 'hover:bg-gray-50'
                  }`}
                >
                  <div className="flex flex-col gap-1">
                    <span className={`font-black ${isUnread ? 'text-red-600' : 'text-gray-900'}`}>
                      عميل: {id.split('_')[1] || id}
                    </span>
                    <span className="text-xs text-gray-500 font-bold uppercase tracking-tighter">ID: {id}</span>
                  </div>
          ...
          <>
            <div className="p-4 bg-gray-50 border-b border-gray-200 flex justify-between items-center">
              <div className="flex items-center gap-3">
                ...
                <span className="font-black text-gray-800 text-lg">محادثة العميل ({activeRoom.split('_')[1]})</span>
              </div>
              <button onClick={() => setActiveRoom(null)} className="text-gray-400 hover:text-red-500 transition-colors font-bold text-sm border px-3 py-1 rounded-lg">إغلاق المحادثة</button>
            </div>
            ...
            <div className="flex-1 p-6 overflow-y-auto bg-[#F8F9FA] space-y-4 flex flex-col">
              {chat.map((m, i) => (
                <div key={i} className={`flex ${m.sender === 'admin' ? 'justify-start' : 'justify-end'}`}>
                  <div className={`p-4 rounded-2xl max-w-md shadow-sm text-sm leading-relaxed ${
                    m.sender === 'admin' 
            ...
            <form onSubmit={sendMsg} className="p-4 bg-white border-t border-gray-200 flex gap-3">
              <input 
